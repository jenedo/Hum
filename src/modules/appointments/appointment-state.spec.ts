...
describe('appointment-state', () => {
  ...
  it.each(validPairs)('allows valid transition %s -> %s', (from, to) => {
    const slotStart = new Date('2026-07-19T09:00:00.000Z');
    const now = new Date(
      slotStart.getTime() + (NO_SHOW_GRACE_MINUTES + 1) * 60_000,
    );

    expect(() => assertTransition(from, to, { now, slotStart })).not.toThrow();
  ...
  it.each(invalidPairs)('rejects invalid transition %s -> %s', (from, to) => {
  ...
  it('rejects NO_SHOW before grace period elapses', () => {
    ...
    expect(() =>
      assertTransition(AppointmentStatus.WAITING, AppointmentStatus.NO_SHOW, {
        now,
        slotStart,
      }),
  ...
  it('allows NO_SHOW after grace period', () => {
    const slotStart = new Date('2026-07-19T09:00:00.000Z');
    const now = new Date(slotStart.getTime() + NO_SHOW_GRACE_MINUTES * 60_000);
